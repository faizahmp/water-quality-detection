...
while (time.time() - start_time) < DURATION_SEC:
    ...
    try:
        ...
        if line:
            match = re.match(
                r"Raw:\s*(\d+)\s*\|\s*Voltage:\s*([\d.]+)\s*V\s*\|\s*TDS:\s*([\d.]+)\s*ppm",
                line,
            )
            if match:
                raw_val = int(match.group(1))
                voltage = float(match.group(2))
                tds = float(match.group(3))
                ...
                if timestep // SAMPLE_RATE_HZ > last_print_time:
                    print(
                        f"[{timestep // SAMPLE_RATE_HZ}s] Raw: {raw_val}, V: {voltage:.3f}, TDS: {tds:.1f} ppm"
                    )
